...
class DigitalBrainModel:
    ...
    def change_password(self,username,new_password):
        logging.info('agent_login method called to check authentication for agent')
        res  = graph.run("MATCH (j:Syslogin {username:{username}}) set j.password = {new_password} return (j)", username = username,new_password=new_password).data()
        print (res)
        if res is not None:
            return jsonify({'password_changed':'true'}),200
        else:
            return jsonify({'password_changed':'false'}),501
    ...
    def get_city_events(self,city):
        res=graph.run("MATCH (a:Geo_Events) where a.rank >=80 RETURN a ORDER BY a.rank DESC")
        ret=[]
    ...
    def get_customer_details(self, policy_id):
        res = graph.run("MATCH (j:Customer) WHERE j.policy_id = {policy_id} RETURN j", policy_id = policy_id)
        res = list(res)
        return jsonify([
            {
    ...
    def get_dependent_details(self, policy_id):
        res = graph.run("MATCH (a:Customer {policy_id: {policy_id}})<-[:IS_DEPEDENDENT_UPON]-(j) RETURN j", policy_id = policy_id)
        res = list(res)
        return jsonify([
            {
    ...
    #     temp['new2'] = m
    #     tx = graph.begin()
    #     graph.run("match(n:Claim {claimid: 'CL1588083219359'}) set n.new1 = {new1}, n.new2 = {new2}", new1 = temp['new1'], new2 = temp['new2'])
    #     tx.commit()
...
#     def dummy(self):
#         temp = {'id': '1', 'name': 'pam'}
#         claim_request_json = json.dumps(temp)

#         l = ["a", 'b']
#         m = {}
#         m["path"] = l
#         claim_request_json = json.dumps(m)

#         tx = graph.begin()
#         graph.run("CREATE (j:Person {descriptiom: {claim_request_json}})", claim_request_json = claim_request_json)
#         tx.commit()
...
#     def test(self, policy_id):
#         a = graph.run("MATCH (j:Policy) WHERE j.policy_id = {policy_id} RETURN j.drivercomprehensive_ded_w_glass_name", policy_id = policy_id).evaluate()
#         b = 100
#         d = '$2,00'
#         e = int(d[1:].replace(",",""))
#         c = '$' + str((int(a[1:].replace(",",""))) - e)
#         s = self.get_customer_address(policy_id)
#         print('---------')
#         print(s)
#         print(s["mailing_address_line_1"])
